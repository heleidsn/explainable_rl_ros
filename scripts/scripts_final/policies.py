'''
Modified from 2.10.1 to fit explainable
'''
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            if self.feature_extraction == "cnn":
                pi_h, layers = self.cnn_extractor(obs, **self.cnn_kwargs)
            else:
                pi_h = tf.layers.flatten(obs)

            # new for grad_cam
            self.feature_befor_fc = pi_h
            self.last_conv_layer = layers[2]
            self.conv_layers = layers

            self.fc_layer_1 = tf.nn.relu(tf.layers.dense(pi_h, 64, name='fc/fc1'))
            self.fc_layer_2 = tf.nn.relu(tf.layers.dense(self.fc_layer_1, 32, name='fc/fc2'))
            self.policy = policy = tf.layers.dense(self.fc_layer_2, self.ac_space.shape[0], activation=tf.tanh, kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.1))

            self.last_conv_layer_grad = tf.gradients(policy, layers[2])  # 最后一层CNN相对于action的导数
            self.grad_image = tf.gradients(self.last_conv_layer, obs)  # gradients from input image to action
            self.grad_feature = tf.gradients(policy, self.feature_befor_fc) # 1*20向量，每个feature相对于action的导数
            self.last_conv_feature_grad = tf.gradients(pi_h, layers[2]) # 最后一层CNN相对于feature的导数
            # TODO: get single gradient
            self.grad_feature1 = tf.gradients(policy[0][0], self.feature_befor_fc)
            self.grad_feature2 = tf.gradients(policy[0][1], self.feature_befor_fc)
            self.grad_feature3 = tf.gradients(policy[0][2], self.feature_befor_fc)
