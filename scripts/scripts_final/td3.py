...
class TD3(OffPolicyRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.l2_loss = l2_loss = tf.losses.get_regularization_loss()

                    # Policy loss: maximise q value
                    self.policy_loss = policy_loss = -tf.reduce_mean(qf1_pi) + l2_loss
    ...
    def network_visualization_semantic(self, obs, action):

        feed_dict = {self.observations_ph: obs[None], 
                     self.actions_ph: action[None]
                     }

        step_ops = [
            self.policy_tf.last_conv_layer,
            self.policy_tf.last_conv_layer_grad,
            self.policy_tf.last_conv_feature_grad,
            self.policy_tf.conv_layers,
            self.policy_tf.feature_befor_fc,
            self.policy_tf.grad_feature,
            self.policy_tf.policy,
            self.policy_tf.grad_feature1,
            self.policy_tf.grad_feature2,
            self.policy_tf.grad_feature3
        ]

        out = self.sess.run(step_ops, feed_dict=feed_dict)

        output_dict = {
            'output_last_conv_layer': out[0][0],
            'grad_lastconv_action': out[1][0][0],
            'grad_lastconv_feature': out[2][0][0],
            'cnn_layer_1': out[3][0][0],
            'cnn_layer_2': out[3][1][0],
            'cnn_layer_3': out[3][2][0],
            'feature_before_fc': out[4][0],
            'grad_feature_action': out[5][0][0],
            'policy_out': out[6][0],
            'grad_feature_action1': out[7][0][0],
            'grad_feature_action2': out[8][0][0],
            'grad_feature_action3': out[9][0][0]
        }

        return output_dict

    def action_analyze_visualization(self, obs):
        '''
        对action进行遍历，得到每个aciton在每个Q-value network下的值
        并对其进行可视化
        目前只输出q_rd_1, q_rd_2和q_reward_sum_out在同一个observation下不同action的值
        '''
        # create action matrix 注意action_ph需要的是scale_action 也就是0-1之间
        actions = self.actions_list
        obs_list = [obs for i in range(len(actions))]

        # get Q-values
        feed_dict = {self.observations_ph: obs_list, 
                     self.actions_ph: actions
                     }

        out = self.sess.run([self.q_rd_1, self.q_rd_2, self.q_reward_sum_out], feed_dict=feed_dict)

        # visualize
        image1 = out[0].reshape(self.x_num, self.y_num)
        image2 = out[1].reshape(self.x_num, self.y_num)
        image3 = out[2].reshape(self.x_num, self.y_num)

        return image1, image2, image3

    def get_feature_output(self, obs):
        '''
        ...
        ops = self.policy_tf.feature_befor_fc
        
        out = self.sess.run(ops, feed_dict=feed_dict)
